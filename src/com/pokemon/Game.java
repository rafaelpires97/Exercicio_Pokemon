...
public class Game {
    ...
    public void runGame() {
        ...
        do{
            ...
            //After the splitting I run a method to verify if the string covers the input requirements (N, S, E ,O)
            if(validString(separatedString)){
                ...
                if(input.equals("QUIT")){
                    //If the user writes QUIT the program ends.

                    //Also this if is here if the user writes "QUIT" the program has to run the whole code to get this to finish,
                    //And for that It would print "Invalid Input, Try Again" line and that is why I put it here.
    ...
    public int playGame(String[] separatedString, HashSet<Coordenadas> visitedCoords){
        ...
        //After the input is verified, this method is called

        //The first thing I do here is to set the starting point of Ash
        //Next and because it is a rule of the game I set the pokemon count already at 1
        //because Ash is already in a specific location of the grid
        ...
        //I add the visited coordinate to the hashset so if he goes there again I can check that he has been there
        visitedCoords.add(cords);


        //Here I will start the movements for Ash, iterating the array
        for (String s : separatedString) {

            //Before I know where he will be moving I need to know where he stands, so I grab the last coordinate known
